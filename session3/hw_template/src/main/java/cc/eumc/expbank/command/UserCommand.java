                ...
                case "withdraw" -> {
                    ...
                    if (args.length == 2) {
                        ...
                        try {
                            ...
                            int amount = Integer.parseInt(amountString); // 42

                            // 获取银行中玩家的余额
                            // ⚠️ 注意：一般情况下不建议在配置文件中存放此种类型的玩家数据
                            // 下次我们将会优化存储方式
                            int balance = plugin.getConfig().getInt("Wallet." + player.getUniqueId().toString() + ".Balance", 0);
                            ...
                            if (balance < amount) {
                                ...
                                // balance >= amount
                                // 从银行中扣除对应经验
                                plugin.getConfig().set("Wallet." + player.getUniqueId().toString() + ".Balance", balance - amount);
                                // 保存配置文件
                                plugin.saveConfig();
